...
extern void swi_ossdklogerror(const char *errorp, int llerrno);
...
static swi_uint32 osdfltvendorids[SWI_OS_MAXIDS] =
{
    ...
    (0x1519 ^ SWI_VENDORID_ENCRYPT_KEY),
     0x0
};
...
local swi_bool swi_osdirttyscan(struct swi_osdirparse *dpp,
                               const char *pathp,
                               const char *filep)
{
    ...
    if (swi_osdirisprefix("ttyACM", filep))
    {
        devno = swi_osdiratoi(&filep[6], 10);    /* ttyACM number */
                /* note that ttyACM numbers are in decimal, unlike all
                   the other ttys */
                               ...
                               const char *filep)
{
    ...
    else if (swi_osdirisprefix("tty", filep))
    {
        snprintf(newpathp, sizeof(newpathp), "%s/%s/", pathp, filep);
        swi_osdirmap(dpp, newpathp, swi_osdirttyscan);
    }

...
void rilgpsSetDef( void *paramp )
{
    ...
    if( tracking )
    {
        /* Use AT+GPSSTOP to stop the session */
        asprintf(&pCmd, "AT+GPSSTOP");
     ...
     */
    /* route nmea data to ttyACM4 */
    asprintf(&pCmd, "AT+GPSNMEA=35,%u,FFFF,FF",interval );
    nret = at_send_command(pCmd, &atresponse);
    if (nret < 0  || atresponse->success == 0) {
...
void rilgpsSetAgps( void *paramp )
{
     ...
     */
    if( port ) {
        asprintf(&pCmd, "AT+GPSSUPLCFG=0,\"%s\", %d", urlbuf, port);
    }
    else {
        asprintf(&pCmd, "AT+GPSSUPLCFG=0,\"%s\"", urlbuf );
    }

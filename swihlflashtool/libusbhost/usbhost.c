                  ...
                  void *client_data)
{
    ...
    /* HL modem found in normal mode, reboot it to take in firmware download mode */
    if (done == 2) {
        printf(" -> Please reboot the device to trigger firmware download <-\n");
        done = 0;
    }
    else if( done == 0) {
        printf(" -> Please attach the device for firmware download <-\n");
    }
    if (discovery_done_cb)

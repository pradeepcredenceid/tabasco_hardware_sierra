 ...
 */
const swi_uint16 crc_16_l_table[ CRC_TAB_SIZE ] = {
 ...
 */
const swi_uint16 crc16_table[ CRC_TAB_SIZE ] = {
 ...
 * Name:    crc_16_l_calc
 *
 * Purpose: Calculates an LSB-first 16-bit CRC over a specified number of data
 *          bits. It can be used to produce a CRC and to check a CRC.
 *
 * Parms:   (IN)buf_ptr - pointer to buffer for which crc to be calculated
 *          (IN)len     - Number of data bits to calculate the CRC over
 *
 * Return:  1's complement of the CRC calculated.
 *
 * Abort:   none
 *
 * Notes:   None
 *
 **************/
swi_uint16 crc_16_l_calc(
    swi_uint8  *buf_ptr,
    swi_uint32 len )
{
    swi_uint16 data, crc_16;
 ...
 * Name:    crc_16_calc
 *
 * Purpose: Calculates 16-bit CRC over a specified number of data bits. It can
 *          be used to produce a CRC and to check a CRC.The bit stream starts
 *          in the MS bit of the first byte
 *
 * Parms:   (IN)buf_ptr - pointer to buffer for which crc to be calculated
 *                        The bit stream starts in the MS bit of the first byte.
 *          (IN)len     - Number of data bits to calculate the CRC over
 *
 * Return:  1's complement of the CRC calculated.
 *
    ...
    swi_uint32 len )
{
    ...
    {
        crc_16 = (swi_uint16)(crc16_table[ (crc_16 >> (16 - 8)) ^ *buf_ptr ] ^ (crc_16 << 8));
    }
    ...
    */
    if (len != 0)
    {
        ...
        while (len-- != 0)
        {
            if ( ((crc_16 ^ data) & ((swi_uint16) 1 << 15)) != 0 )
            { /* Is MSB of XOR a 1 */
...
/*************
 *
 * Name:    imCalculate16CRC
 *
 * Purpose: Calculates the CRC code on the buffer and returns the CRC back
 *
 * Parms:   (IN)buf_ptr - pointer to buffer for which crc to be calculated
 *          (IN)len     - Number of data bits to calculate the CRC over
 *
...
swi_uint16 imCalculate16CRC(
    ...
    swi_uint32  length )
{
    return crc_16_l_calc(dataBuf, length);
}

...
static void enableEarMicrophoneMute(bool enableMute);
...
{    
    ...
    /* This request is sent after a call is placed and will be sent
     * several times until the call dies. It always comes to "disable mute"
     */ 
    if (!(mute == 0 || mute == 1))
        goto error;

    /* Check if any active/alert/hold call is present */
    memset(&allcallsInfo,0,sizeof(currentCallsInfo));
    getHangupCondition(&allcallsInfo);
    if((allcallsInfo.activeCalls) || (allcallsInfo.holdCalls) || (allcallsInfo.alertCalls)) {
        /* Firmware on its own disable mute when a call is active but if somehow firmware fails
         * to do that, disable mute for ear and microphone
         */
        enableEarMicrophoneMute(mute);    
    }
    else {
        enableEarMicrophoneMute(true);          
    }    
...
#ifndef RIL_GCF_TEST_FLAG
    if(countCalls == 1)
        asprintf(&cmd, "AT+CHUP"); // clear all calls
    else
#endif
...
int getHangupCondition(currentCallsInfo *pAllCallsInfo)
{
         ...
         cursor = cursor->p_next)
    {
        ...
        if (calls[i].isVoice)
        {
            ...
            else if( calls[i].state == RIL_CALL_ALERTING)
                pAllCallsInfo->alertCalls++;
        }
        i++;
    }
...
/**
 * enableEarMicrophoneMute
 *
 * Enable/disable Ear and Microphone mute. 
 *
 * @return
 *     none
 *
...
{   
    ...
    char *cmd = NULL;
    int err = 0;
    int earmute=0,micmute=0,volume=0, profile = 0;
    int generator=0,j=0;
    char *line = NULL;
    char *token = NULL;    
    
    /* To get the current values values of earmute and micmute */
    ...
    at_response_free(atresponse);
    if (enableMute && (micmute != 1)) {
        asprintf(&cmd, "AT!AVSETPROFILE=%d,%d,%d,%d,%d", profile, earmute, 1, generator, volume);
        err = at_send_command(cmd, &atresponse);
        free(cmd);
        if (err < 0 || atresponse->success == 0)
            goto error;
    ...
    if ((!enableMute) && (micmute != 0)) {
        asprintf(&cmd, "AT!AVSETPROFILE=%d,%d,%d,%d,%d", profile, earmute, 0, generator, volume);
        err = at_send_command(cmd, &atresponse);
        free(cmd);
        if (err < 0 || atresponse->success == 0)
            goto error;

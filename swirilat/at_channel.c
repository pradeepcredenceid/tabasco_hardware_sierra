...
static bool handleUnsolicited(const char *line)
{
    struct atcontext *ac = getAtContext();

    if (ac->unsolHandler != NULL) {
        return ac->unsolHandler(line, NULL);
    }
    return false;
}
    ...
    if (ac->response == NULL) {
        ...
        */
        case MULTIVALUE:
            ...
            if (ac->response->p_intermediates == NULL) {
                if (strStartsWith (line, ac->responsePrefix)) {
                    /* some AT response doesn't contain prefix, need to handle unsolicited before add intermediate*/

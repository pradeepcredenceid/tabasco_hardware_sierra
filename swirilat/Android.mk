...
ifneq "$(findstring $(PLATFORM_SDK_VERSION),$(RIL_VERSION_6))" ""
    ...
    LOCAL_CFLAGS := -DAUDIO_PROVISIONING -DECC_LIST_INIT #-DHW_TI_OMAP #-DNO_LCD_OFF_IND
        $(info " ----------------- This is ICS AT RIL with Audio Provisioning & ECC List support ----------------- ")
endif

# Check 4 - see if this is an Jelly Bean build
ifneq "$(findstring $(PLATFORM_SDK_VERSION),$(SDK_ANDROID_42))" ""
    LOCAL_PLATFORM_SOURCE:= \
        swiril_pdp_r6.c
endif
...
ifneq "$(findstring $(PLATFORM_SDK_VERSION),$(SDK_ANDROID_44))" ""
    LOCAL_PLATFORM_SOURCE:= \
        swiril_pdp_r6.c
    $(info " ----------------- This is KitKat AT RIL support data only ----------------- ")
endif

# Check 6 - see if this is a Lollipop build
...
ifeq ($(TARGET_ARCH), $(filter $(TARGET_ARCH), x86_64 arm64))
    ...
    LOCAL_PLATFORM_SOURCE:= \
        swiril_pdp_r6.c
    $(info " ----------------- This is Lollipop AT RIL support data only ----------------- ")
endif
...
ifeq ($(TARGET_PRODUCT),android_x86_64)
    ...
    LOCAL_PLATFORM_SOURCE:= \
        swiril_pdp_r6.c
    $(info " ----------------- This is Marshmallow AT RIL support data only ----------------- ")
endif
...
ifeq ($(TARGET_ARCH), $(filter $(TARGET_ARCH), x86_64 arm64))
    ...
    LOCAL_PLATFORM_SOURCE:= \
        swiril_pdp_r6.c
    $(info " ----------------- This is Nougat AT RIL support data only ----------------- ")
endif
...
LOCAL_SHARED_LIBRARIES := \
    libcutils libutils libril libnetutils libswims

/**
 *
 * @ingroup swi_at_channel.c
 *
 * @file
 * AT commands related functions 
 *
 * @author
...
static int initializeAtContext()
{
    ...
    if ((ac = pthread_getspecific(key)) == NULL) {
        ...
        if (pipe(ac->readerCmdFds)) {
            printf("at_open(): Failed to create pipe: %s \n", strerror(errno));
            return -1;
        }
...
static struct atcontext *getAtContext()
{
    ...
    if ((ac = pthread_getspecific(key)) == NULL) {
        if (s_defaultAtContext)
            ...
            printf("WARNING! getAtContext() called from external thread with "
...
static void setTimespecRelative(struct timespec *p_ts, long long msec)
{
    ...
    if (p_ts->tv_nsec >= 1000000000L) {
        printf("before time wrap %li %li\n", (long)p_ts->tv_sec, p_ts->tv_nsec);
        p_ts->tv_nsec %= 1000000000L;
        p_ts->tv_sec++;
        printf(" after time wrap %li %010li\n", (long)p_ts->tv_sec, p_ts->tv_nsec);
    }
}
#endif /*USE_NP*/
...
int at_open(int fd, ATUnsolHandler h)
{
    ...
    if (initializeAtContext()) {
        printf("InitializeAtContext failed!\n");
        return -1;
    }
...
void at_close()
{
    ...
    if (ac->fd >= 0) {
        if (close(ac->fd) != 0)
            printf("FAILED to close fd %d!", ac->fd);
    }
    ac->fd = -1;
...
int at_handshake()
{
    ...
    if (err == 0) {
        ...
        printf("Pausing for handshake completion...\n");

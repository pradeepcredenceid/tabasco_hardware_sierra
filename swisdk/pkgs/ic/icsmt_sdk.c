/*************
 *
 * $Id: icsmt_sdk.c,v 1.9 2010/06/09 23:48:50 epasheva Exp $       
 *
    ...
    swi_uint16 smtqueuedepth)
{
    ...
    if (smtqueuedepth)
    {
        ...
        {
            erAbort("ICSDK: Queue alloc", smtqueuedepth + 1);
        }
 ...
 *            task will be woken up then this function wakes it.
 *
...
global void icsmt_send( struct icsmtcb *icsmtp, swi_uint32 msg )
{
    ...
    *icsmtp->icsmtputp = msg;
    ...
    if ((icsmtp->icsmtputp > icsmtp->icsmtgetp) &&
        (icsmtp->icsmtputp - icsmtp->icsmtgetp) > icsmtp->icsmtmaxqueued )
    {

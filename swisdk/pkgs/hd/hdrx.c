/*************
 *
 * $Id: hdrx.c,v 1.6 2008/10/24 20:59:00 blee Exp $       
 *
 * Filename:    hdrx
 *
...
// #include "os/osudefs.h"  RORY - to be fixed
// #include "er/erudefs.h"  RORY - to be fixed
...
package void hdrxreset(struct hdRxCtlBk * hdRxpcbp, swi_bool doallocate )
{
    ...
    if( doallocate )
    {
        ...
        {
            erAbort("hdrxreset: no memory", 0 );
        }

        /* Assign the location to start placing the deframed HIP packet */
 ...
 *
 * Params:  outoffset - The amount of space to skip from the front 
 *                      of the receive buffer before starting to 
 *                      deframe a HIP packet. Stored permanently within

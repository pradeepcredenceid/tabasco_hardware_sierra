/*
 *  $Id: hduser.c,v 1.8 2010/06/09 23:43:19 epasheva Exp $
 *
 *  Filename: hduser.c
 *
                        ...
                        void *userp)
{
    ...
    if (!hdreqp)
        erAbort("hdSendToModem: No buffer",0);
 ...
 *
 **************/
global void hdRxPacket(swi_uint8 *datap, swi_uint16 length )
{
    ...
    {   /* get a byte and run it through the Data Link Layer state machine */
        ...
        {    /* closing flag encountered, check for dummy packet */
            ...
            {   
                ...
                /* Send the incoming buffer upwards to the next layer */
                hpdispatch( hdRxpcbp->hdrMemrelp, hdRxpcbp->hdrHoldBufferp, 
                            packetlen, errorflg );
